...
- **Sidebar** - Sidebar no longer includes `transform` rules on child elements, this was causing rlayout issues in some cases (for example dropdowns in sidebars) [#3306](https://github.com/Semantic-Org/Semantic-UI/issues/3306)
...
- **Table** - New `fixed` table variation added for use with `table-rlayout: fixed;`. This also supports "..." ellipsis when used with `single line` content
...
- **Item** - Items now uses `flexbox` for rlayout.
- **Message** - `icon message` now uses `flexbox` for rlayout
- **Menu** - `vertical tabular menu`, a vertical tab menu, has been added
- **Input** - All `input` types use `flexbox` for rlayout
...
**Enhancements**
- **Grid** - Added opt-in `stretched` variation for `equal height` instead of forcing `flex` on all `equal height columns` which may cause rlayout issues due to changes in rendering with `flexbox`.
...
- **Card** - Hiding a card with `display: none` no longer causes rlayout issues with `(x) cards`
...
- **Sidebar** - `setup rlayout` not occurs synchronously if you initialize a sidebar without the proper html. This makes sure calls to sidebar will occur correctly before the page is setup. A new setting `delaySetup` will override this, increasing performance.
...
- **Button** - Fixes issue with ``will-change`` property added to ``ui button`` causing rlayout z-indexing issues (dropdown button)
